...
class BibleVersePresentationBuilder(PresentationBuilder):
    ...
    def get_passages(self):
        passage = Passage(book=BibleBook.exodus, start_verse=Verse(
            chapter=7, verse=3), end_verse=Verse(chapter=7, verse=4))
        return [passage]
    ...
    def cover_one(self):
        ...
        # TODO: if else check start and end chapter same or not
        return [{
            'cover_one': '读经',
...
slide = BibleVersePresentationBuilder(base_name="base_wide.pptx")
slide.build()
slide.save_file()
